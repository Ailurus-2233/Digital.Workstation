...
MainWindowViewModel.cs              主窗口 ViewModel（365 行）：布局状态机驱动 + 面板对齐档位 + 贡献收集 + 视图缓存
...
- **MainWindowViewModel.cs**：构造（:30-39）；`EnsureContributionsLoaded`（:119-150——`GetToolViews()` 一次拉出工具视图（:127）按 `Placement`/`AllowMove` 分派，菜单为 `MenuTreeBuilder.Build(_collector.GetMenuItems())` 建树 + `MenuItemViewModel.FromSubmenu` → `MenuBarItems`，:142-145）；命令/方法 `SelectActivity`（:156）、`OpenMainView`（:182）、`ActivateAuxTab`（:203）、`ActivateBottomTab`（:220）、`ToggleSideBar`（:237）、`ToggleAuxiliaryPanel`（:246）、`ToggleBottomPanel`（:255）、`SetPanelAlignment`（:264）、`ResizePanel(PanelResize)`（:275）、`TogglePanel`（:283）；私有装载 `LoadPanelTabs`（:296）、`SyncActiveTab`（:332）、`LoadItems`（:355）。字段与缓存字典清单见 reference.md。
