...
构造注入：`MainWindowViewModel(ShellContributionCollector collector, IContainerProvider containerProvider, IEventAggregator eventAggregator, LayoutPersistence persistence)`（:37-50；`LayoutPersistence` 类型在 Framework：`Core/Framework/Layout/LayoutPersistence.cs`，经 `using DigitalWorkstation.Core.Framework.Layout` 解析）。由 Prism ViewModelLocator 自动装配（`MainWindow.axaml:12` `prism:ViewModelLocator.AutoWireViewModel="True"`）。构造函数订阅 `OpenMainViewEvent`→`OpenMainView`、`TogglePanelVisibilityEvent`→`TogglePanel`、`SetPanelAlignmentEvent`→`SetPanelAlignment`、`ResetLayoutEvent`→`ResetLayout`（:43-46），订阅 Framework `ToolViewDragSession.ActiveChanged` 写 `IsToolViewDragActive`（:48，拖拽会话期间临时显露隐藏面板，ADR-0002），并把 `_mainContent` 初始化为 `containerProvider.Resolve<EmptyStateView>()`（:49）。

### 可绑定属性（XAML 消费面，绑定点见 MainWindow.axaml 与 Framework 的 `FrameworkWindowTheme.axaml`——后者是宽松反射绑定，Framework 不引用本类型）

| 属性 | 类型 | 语义 |
|---|---|---|
| `State` | `ShellLayoutState`（[ObservableProperty]，:55，初值 `ShellLayoutState.Initial`；带 `[NotifyPropertyChangedFor(nameof(SideBarColumnWidth), nameof(AuxiliaryColumnWidth), nameof(AuxiliaryPanelRevealed), nameof(BottomPanelRevealed))]`，:52-54） | 整个布局状态：ActivityBar 顶部段顺序、面板显隐/宽高、选中导航项、活动 tab、活动主视图。**面板对齐档位不在其中**（见下行） |
| `PanelAlignment` | `PanelAlignment`（[ObservableProperty]，:80，初值 `PanelAlignment.Center`） | 面板对齐档位（左/右/居中/两端）：与 `FrameworkWindow.PanelAlignment` 双向绑定的镜像属性——窗口依赖属性是布局定义的唯一入口（:75-78 注释）；由 `SetPanelAlignmentEvent` 订阅写入 |
| `IsToolViewDragActive` | `bool`（[ObservableProperty]，:63；带 NotifyPropertyChangedFor 两个 Revealed 属性与 `AuxiliaryColumnWidth`，:61-62） | 工具视图拖拽会话进行中（Framework `ToolViewDragSession` 驱动） |
| `AuxiliaryPanelRevealed` / `BottomPanelRevealed` | `bool`（:68、:73） | 面板是否应当呈现：持久可见（`State.X.Visible`）**或**拖拽会话期间临时显露以接受投放；面板卡片 `IsVisible` 绑这两个属性而非直接绑 State |
| `SideBarContent` / `SideBarTitle` | `object?` / `string?`（:83、:91） | SideBar 当前内容与区域标题 |
| `MainContent` | `object`（:88） | 主区当前内容；初始为 `EmptyStateView`，被 `OpenMainView` 整体替换 |
| `AuxiliaryContent` / `BottomContent` | `object?`（:115、:121） | 两个面板当前活动 tab 的内容 |
| `TopNavigationItems` / `BottomNavigationItems` | `ObservableCollection<NavigationItemViewModel>`（:93、:95） | ActivityBar 顶部/底部导航项（顶部段顺序与 `State.ActivityBarItems` 同步） |
| `AuxiliaryTabs` / `BottomTabs` | `ObservableCollection<PanelTabViewModel>`（:96、:98） | 两个面板的 tab 栏（顺序与 `State.X.Tabs` 同步） |
| `MenuBarItems` | `ObservableCollection<MenuItemViewModel>`（:103；`MenuItemViewModel` 类型在 Framework：`Core/Framework/Menus/MenuItemViewModel.cs`，经 `using DigitalWorkstation.Core.Framework.Menus` 解析） | 菜单栏（ADR-0001）：全部菜单贡献经 `MenuTreeBuilder.Build` 建树生成，顶层菜单与子菜单节点同为 `MenuItemViewModel`，分隔线以 Avalonia `Separator` 控件形式存在于子级 `Children`；渲染侧是 `FrameworkWindow` 内置的标题栏菜单栏（`Core/Framework/Windows/FrameworkWindow.cs:35-41` 宽松绑定 `MenuBarItems`，chrome-menu 样式在 `Core/Framework/Windows/FrameworkWindowTheme.axaml:598-616`） |
| `StatusBarItems` | `ObservableCollection<StatusBarItemViewModel>`（:109） | 状态栏条目 |
| `CollapseBottomIcon` / `CollapseAuxiliaryIcon` | `Geometry`（:125 `Icons.ChevronDown`、:130 `Icons.ChevronRight`） | 两个面板收起按钮图标 |
| `SideBarColumnWidth` / `AuxiliaryColumnWidth` | `GridLength`（:136-137、:142-143） | 可见（Auxiliary 取 Revealed）时面板宽度 + 4px 间隙（布局模板间隙约定），隐藏归零使 BottomPanel 跨度自然伸缩；供 Framework 布局模板的列宽绑定 |

### 命令（[RelayCommand] 生成，XAML 绑定名 = 方法名 + Command）

| `SelectActivityCommand` | `SelectActivity(NavigationItemViewModel)`（:269） | 选中导航项并驱动 SideBar 展开/收起：`State = State.SelectActivity(item.Id)` 后调 `SyncSideBarSelection()`（:279，统一出口：遍历 `_itemsById.Values` 同步 `IsSelected`；SideBar 可见且 `ContentFor` 有对应贡献时设置 `SideBarTitle` 与缓存内容）；末尾 `ScheduleSave()` 防抖落盘（:272） |
| `ActivateAuxTabCommand` | `ActivateAuxTab(PanelTabViewModel)`（:333） | 激活 AuxiliaryPanel tab：先 `State.ActivateAuxTab(tab.Id)` 得 `next`；面板收起时 `ShellLayoutState` 拒绝并原样返回原实例，`ReferenceEquals(next, State)`（:336）为 true → 直接 return，不赋 `State`、不调 `SyncPanelTab`、不落盘；状态变化时 `State = next` 并调 `SyncPanelTab(ToolViewPlacement.AuxiliaryPanel)`（:341）同步各 tab 的 `IsActive` 与缓存的内容视图，末尾 `ScheduleSave()`（:342） |
| `ActivateBottomTabCommand` | `ActivateBottomTab(PanelTabViewModel)`（:350） | 同上，针对 BottomPanel（`State.ActivateBottomTab(tab.Id)`，:353；`ScheduleSave()` 在 :359） |
| `ToggleSideBarCommand` | `ToggleSideBar()`（:367） | Ctrl+B；转发 `TogglePanel(TogglePanelTarget.SideBar)` |
| `ToggleAuxiliaryPanelCommand` | `ToggleAuxiliaryPanel()`（:376） | Ctrl+Alt+B 或收起按钮 |
| `ToggleBottomPanelCommand` | `ToggleBottomPanel()`（:385） | Ctrl+J 或收起按钮 |
| `ResizePanelCommand` | `ResizePanel(PanelResize)`（:406） | 分隔条拖拽的唯一路径：Framework 的 `PanelResizer` 经 `Target` + `ResizeCommand` 声明式调用（code-behind 不再参与拖拽）；`State = State.Resize(resize.Target, resize.Delta)`，增量经状态转换应用并 clamp 到合法区间；面板收起时尺寸记录保留；末尾 `ScheduleSave()`（:409）。面板对齐切换不是命令：视图菜单对齐项发布 `SetPanelAlignmentEvent`，构造函数订阅（:45）调私有 `SetPanelAlignment`（:394）写入 `PanelAlignment` 镜像属性并 `ScheduleSave()`（:397） |
| `MoveTabCommand` | `MoveTab(ToolViewMove)`（:417，ADR-0002） | 工具视图拖拽落放的唯一路径：Framework 的 `ToolViewBar` 在 Drop 时经 `MoveCommand` 声明式调用。钉住项/未知 Id 拒绝（:419-422，`AllowMove=false` 不发起状态转换）；`State.MoveTab` 拒绝（原地落放等）时 `ReferenceEquals` 早退（:425-428）；**内容实例随 tab 走**：先把被拖内容从三个显示区属性置空脱离源视觉树（:431-445，同一 Control 不能同时挂两棵视觉树），再 `State = next`（:447），经 `SyncBarCollection` 把三个 Bar 集合对齐到 State 的有序 Id 列表（:449-454），最后 `SyncSideBarSelection` + 两个 `SyncPanelTab` 同步高亮与内容（:456-458），`ScheduleSave()` 落盘（:459） |

### 公开方法

| 方法 | 签名/位置 | 调用方 |
|---|---|---|
| `EnsureContributionsLoaded` | `public void`（:149-171） | 仅 `MainWindow.axaml.cs:13-17` 的 `OnOpened`；`_contributionsLoaded` 守卫保证只执行一次。方法体内先 **`GetToolViews()` 一次**拉出全部工具视图贡献存入 `_toolViews` 字段缓存（按 `Order` 升序，ADR-0002，:157），随后 **`LoadToolViews(_persistence.Load())`**（:158）——读持久化布局（缺失/损坏/版本不符返回 null）并按「配置优先、默认兜底」分派三处 Bar（详见私有方法 `LoadToolViews`）；**`GetMainViews()`** 把每个 `IMainViewContribution` 按 `Id` 存入 `_mainViewsById`（:159-162）；**`GetMenuItems()`**（无参，不过滤不排序，ADR-0001）经 Framework 的 `MenuTreeBuilder.Build` 建树（顶层排序不分组、子菜单分组排序且组间自动插分隔线），逐顶层节点 Framework 的 `MenuItemViewModel.FromSubmenu` 递归转换加入 `MenuBarItems`（:163-166）；**`GetStatusBarItems()`** 逐项包装为 `StatusBarItemViewModel` 加入 `StatusBarItems`（:167-170） |

### 私有方法（改行为时直接面对）

`TogglePanel(TogglePanelTarget)`（:465-474，switch：SideBar→`State.ToggleSideBar()`、AuxiliaryPanel→`State.ToggleAuxiliaryPanel()`、默认 `_`→`State.ToggleBottomPanel()`，末尾 `ScheduleSave()` :473——快捷键/菜单/按钮三路显隐切换统一在此落盘）、`SetPanelAlignment(PanelAlignment)`（:394-398，事件处理，`PanelAlignment = alignment` + `ScheduleSave()`）、`OpenMainView(string viewId)`（:312-327，未知 Id 静默返回）、**`LoadToolViews(ShellLayoutDto?)`**（:178-208：先把全部贡献登记进 `_contributionsById`（:161-164）；钉住项（`AllowMove=false`）恒落 ActivityBar 底部段（:194-195）；可移动项「配置优先、默认兜底」——局部函数 `MovableIn(bar)`（:166-176）先取持久化 `placements` 里归属该 bar 的（按 `Index` 排序），再把无配置条目且 attribute `Default == bar` 的按 `Order` 追加；孤儿条目随贡献迭代自然丢弃，无配置的新工具视图落到 Default Bar 末尾。ActivityBar 顶部段顺序写入 `State.ActivityBarItems`（:179-180）后 `LoadItems` 填两个导航集合、`LoadPanelTabs` 填两个面板（透传 `layout?.XxxPanel?.ActiveTab` 作 `preferredActiveTab`）；`layout` 非 null 再 `RestoreLayout`（:220）恢复显隐/尺寸（clamp）/选中项（必须是顶部段当前成员，否则按孤儿丢弃）/对齐档位）、**`ResetLayout()`**（:480-498，删持久化文件、清集合与索引、State/对齐回初值、`LoadToolViews(null)` 全默认重建；`_toolViewContents` 视图实例缓存保留）、**`CaptureLayout()`**（:504-550，快照当前布局为 DTO）、**`ScheduleSave()`**（:555-558，统一防抖落盘出口）、**`LoadPanelTabs`**（:563-595，建 tab 索引 `_tabsById`、定活动 tab、写 State.Tabs/ActiveTab、调 `SyncPanelTab`）、**`SyncPanelTab(ToolViewPlacement)`**（:601-623，按 State 同步 tab 高亮与内容区，无活动 tab 清空内容）、**`SyncSideBarSelection()`**（:279-295，按 State 同步导航项高亮与 SideBar 标题/内容）、**`ContentFor(string id)`**（:299-309，工具视图内容实例统一入口，`_toolViewContents` 按 Id 单实例缓存，未命中经 `_containerProvider.Resolve(ViewType)` 创建）、**`SyncBarCollection<TItem>`**（:640-678，static：把 Bar 呈现集合对齐到有序 Id 列表——移出删除、缺失经工厂创建并缓存复用、错位 `Move`）、`LoadItems`（:681-689，登记 `_itemsById` 并填导航集合）。
