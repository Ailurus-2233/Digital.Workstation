...
| `SettingsIcon` / `SettingsTitle` | `Geometry` / `string`（:146 `StreamGeometry.Parse(Icons.Settings)`、:151 `Language.SettingsNavigationTitle`） | ActivityBar 底部"设置"入口按钮的图标几何与标题（工具提示/无障碍名）；该按钮是 shell 内置纯导航按钮（非工具视图，ADR-0006 决策 6），绑定点在 Framework 的 `FrameworkWindowTheme.axaml:46-52` |
...
| `ToggleSideBarCommand` | `ToggleSideBar()`（:399） | Ctrl+B；转发 `TogglePanel(TogglePanelTarget.SideBar)` |
| `ToggleAuxiliaryPanelCommand` | `ToggleAuxiliaryPanel()`（:408） | Ctrl+Alt+B 或收起按钮 |
| `ToggleBottomPanelCommand` | `ToggleBottomPanel()`（:417） | Ctrl+J 或收起按钮 |
| `ResizePanelCommand` | `ResizePanel(PanelResize)`（:438） | 分隔条拖拽的唯一路径：Framework 的 `PanelResizer` 经 `Target` + `ResizeCommand` 声明式调用（code-behind 不再参与拖拽）；`State = State.Resize(resize.Target, resize.Delta)`，增量经状态转换应用并 clamp 到合法区间；面板收起时尺寸记录保留；末尾 `ScheduleSave()`（:441）。面板对齐切换不是命令：视图菜单对齐项发布 `SetPanelAlignmentEvent`，构造函数订阅（:50）调私有 `SetPanelAlignment`（:426）写入 `PanelAlignment` 镜像属性并 `ScheduleSave()`（:429） |
| `MoveTabCommand` | `MoveTab(ToolViewMove)`（:449，ADR-0002） | 工具视图拖拽落放的唯一路径：Framework 的 `ToolViewBar` 在 Drop 时经 `MoveCommand` 声明式调用。钉住项/未知 Id 拒绝（:451-454，`AllowMove=false` 不发起状态转换）；`State.MoveTab` 拒绝（原地落放等）时 `ReferenceEquals` 早退（:457-460）；**内容实例随 tab 走**：先把被拖内容从三个显示区属性置空脱离源视觉树（:463-477，同一 Control 不能同时挂两棵视觉树），再 `State = next`（:479），经 `SyncBarCollection` 把三个 Bar 集合对齐到 State 的有序 Id 列表（:481-486），最后 `SyncSideBarSelection` + 两个 `SyncPanelTab` 同步高亮与内容（:488-490），`ScheduleSave()` 落盘（:491） |
| `OpenSettingsCommand` | `OpenSettings()`（:303-306，ADR-0006 决策 6） | ActivityBar 底部"设置"入口（shell 内置纯导航按钮，非工具视图）：经构造缓存的 `_eventAggregator` 发布 `OpenMainViewEvent(WellKnownViews.Settings)`，由本类 `OpenMainView` 订阅打开 Settings 模块贡献的设置页主视图；`WellKnownViews.Settings`（`"settings.main"`）在 Abstractions：`Core/Abstractions/Regions/WellKnownViews.cs`，经 `using DigitalWorkstation.Core.Abstractions.Regions` 解析；不落盘 |
...
`TogglePanel(TogglePanelTarget)`（:497-506，switch：SideBar→`State.ToggleSideBar()`、AuxiliaryPanel→`State.ToggleAuxiliaryPanel()`、默认 `_`→`State.ToggleBottomPanel()`，末尾 `ScheduleSave()` :505——快捷键/菜单/按钮三路显隐切换统一在此落盘）、`SetPanelAlignment(PanelAlignment)`（:426-430，事件处理，`PanelAlignment = alignment` + `ScheduleSave()`）、`OpenMainView(string viewId)`（:344-360，未知 Id 静默返回）、**`LoadToolViews(ShellLayoutDto?)`**（:200-236：先把全部贡献登记进 `_contributionsById`（:204-207）；钉住项（`AllowMove=false`）恒落 ActivityBar 底部段（:225-226）；可移动项「配置优先、默认兜底」——局部函数 `MovableIn(bar)`（:209-219）先取持久化 `placements` 里归属该 bar 的（按 `Index` 排序），再把无配置条目且 attribute `Default == bar` 的按 `Order` 追加；孤儿条目随贡献迭代自然丢弃，无配置的新工具视图落到 Default Bar 末尾。ActivityBar 顶部段顺序写入 `State.ActivityBarItems`（:222-223）后 `LoadItems` 填两个导航集合、`LoadPanelTabs` 填两个面板（透传 `layout?.XxxPanel?.ActiveTab` 作 `preferredActiveTab`）；`layout` 非 null 再 `RestoreLayout`（:242）恢复显隐/尺寸（clamp）/选中项（必须是顶部段当前成员，否则按孤儿丢弃）/对齐档位）、**`ResetLayout()`**（:512-530，删持久化文件、清集合与索引、State/对齐回初值、`LoadToolViews(null)` 全默认重建；`_toolViewContents` 视图实例缓存保留）、**`CaptureLayout()`**（:536-582，快照当前布局为 DTO）、**`ScheduleSave()`**（:587-590，统一防抖落盘出口）、**`LoadPanelTabs`**（:595-627，建 tab 索引 `_tabsById`、定活动 tab、写 State.Tabs/ActiveTab、调 `SyncPanelTab`）、**`SyncPanelTab(ToolViewPlacement)`**（:633-655，按 State 同步 tab 高亮与内容区，无活动 tab 清空内容）、**`SyncSideBarSelection()`**（:311-326，按 State 同步导航项高亮与 SideBar 标题/内容）、**`ContentFor(string id)`**（:331-340，工具视图内容实例统一入口，`_toolViewContents` 按 Id 单实例缓存，未命中经 `_containerProvider.Resolve(ViewType)` 创建）、**`SyncBarCollection<TItem>`**（:672-711，static：把 Bar 呈现集合对齐到有序 Id 列表——移出删除、缺失经工厂创建并缓存复用、错位 `Move`）、`LoadItems`（:657-666，登记 `_itemsById` 并填导航集合）。
