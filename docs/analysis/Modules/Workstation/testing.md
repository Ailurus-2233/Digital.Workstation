...
| `MainWindowViewModel` 交互逻辑（贡献收集、缓存、命令转发、布局持久化接线） | 无单元测试覆盖；手动 `dotnet run` 冒烟：启动 → 点 ActivityBar 导航项（SideBar 展开/再点收起；底部段应只见钉住项"设置"）→ Ctrl+B/Ctrl+J/Ctrl+Alt+B → 拖三条分隔条（边界应停在 Min/Max）→ 视图菜单三个切换项 → 文件>退出、帮助>关于；持久化：改布局后退出重进应恢复 → 视图菜单 Layout 组"重置布局"后应回全默认且 %AppData%/Digital.Workstation/layout.json 被删除 |
