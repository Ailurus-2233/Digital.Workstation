...
| `Modules/DashBoard` | `DashBoardModule`（Prism 模块）、`DashBoardWindow`（启动台窗口） | `WorkstationApplication.cs:18` `AddModule<DashBoardModule>()`、`:41-44` `CreateSplashWindow()` 返回 `Container.Resolve<DashBoardWindow>()` |

### 传递依赖（未在 csproj 直接引用，源码 using 其命名空间）

| 传递来源 | 用到的能力 | 使用点 |
|---|---|---|
| `Core/Abstractions`（经 Framework） | 工具视图契约：`ToolViewAttribute`/`ToolViewContribution`/`ToolViewPlacement`（Abstractions/Contributions/，ADR-0002；旧 `INavigationItemContribution`/`IPanelTabContribution` 与定位枚举已删除）；`IMainViewContribution`/`IStatusBarItemContribution`（Abstractions/Contributions/）；`IMenuItemContribution`、`MenuGroupAttribute`/`MenuItemAttribute`（Abstractions/Menus/）；`ICommandContribution`、`CommandAttribute`（Abstractions/Commands/，ADR-0005）；`IWindowManager`（Abstractions/WindowManager/） | `Views/` 五个 View 类标注 `[ToolView]`；`ReadyStatusBarItem` 实现 `IStatusBarItemContribution`；`MainWindowViewModel.cs:159-189` `LoadToolViews` 按 `ToolViewPlacement`/`AllowMove` 与持久化配置分派收集结果、`:116` `Commands` 属性类型为 `ICommandContribution`；五个菜单类标注两个 attribute（`Abstractions.Menus`）；`Commands/ViewCommands.cs` 四个方法标注 `[Command]`（`Abstractions.Commands`）；`Menus/HelpMenus.cs:12` 注入 `IWindowManager` |
...
| Prism（经 Framework：`Prism.DryIoc.Avalonia`） | `IContainerRegistry`/`IContainerProvider`、`IEventAggregator`、ViewModelLocator（`DelegateCommand` 不再出现于本模块——菜单命令由 Framework 的反射贡献实现包装） | `WorkstationApplication.cs:21、25、29、31、33`；`MainWindowViewModel.cs:33-44`；`Menus/ViewPanelMenus.cs:12`、`ViewAlignmentMenus.cs:13`、`ViewLayoutMenus.cs:11`、`Commands/ViewCommands.cs:10` 构造注入 `IEventAggregator`；`MainWindow.axaml:12` `AutoWireViewModel` |
