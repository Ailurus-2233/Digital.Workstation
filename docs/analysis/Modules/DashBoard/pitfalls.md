...
7. **调整 `Order`/组序值"取整"**：`DashBoardTasksPanelTab.Order=15`、`DashBoardStatusBarItem.Order=20` 与 `DashBoardMenus` 的 `[MenuGroup(GroupOrder=100)]`/`[MenuItem(Order=100)]` 不是随意值，是插进 shell 预置项之间（或预置组之前）的排序验证点（各类注释明示）；改成 0/1000 这类整齐数字会让排序验证失效甚至改变相对位置。
