...
- **启动台窗口不由模块自己打开**：`DashBoardModule.OnInitialized`（DashBoardModule.cs:23-26）是**有意的空方法**，注释写明「启动台窗口由 shell 启动序列在模块加载前显示（ADR-0004），模块自身不再开窗」。原因：启动台必须在模块加载开始前就可见（用来显示模块加载进度本身），此时模块系统尚未初始化本模块，所以由启动序列直接 `Container.Resolve<DashBoardWindow>()`（WorkstationApplication.cs:53）显示。推论：`DashBoardWindow` 与其 ViewModel 不能依赖任何"模块已初始化"的状态。
...
- **贡献是纯声明**：导航项/主视图/面板 tab/状态栏四类贡献类只有属性（`Id`/`Title`/`IconPath`/`Order`/定位枚举/`ContentViewType` 或 `ViewType`），无方法；菜单项改用 attribute 模式（ADR-0001）——`DashBoardMenus` 是不实现接口的普通类，类级 `[MenuGroup]` 声明路径与分组，方法级 `[MenuItem]` 声明标题键/位次/图标，行为直接写在方法体（`OpenDashBoard` 调 `ShowWindow<DashBoardWindow>`），由 Framework 的 `RegisterMenus` 反射注册。视图以 `Type` 而非实例交出，实例化延迟到 shell 侧经容器解析（支持 DI）。排序值刻意插进 shell 预置项之间——面板 tab `Order=15` 介于预置"输出"(10) 与"日志"(20) 之间、菜单所在 General 组 `GroupOrder=100` 小于预置"退出"所属 Application 组（1000）、状态栏 `Order=20` 排在预置"就绪"(10) 之后——用排序结果验证统一排序逻辑（各文件类注释明示此意图）。
...
- **依赖面收窄到 Core**：`DashBoard.csproj`（第 20-23 行）只引用 Core 下的四个项目——`Abstractions`（贡献契约于 `Abstractions/Contributions/`、菜单 attribute 于 `Abstractions/Menus/`、`IWindowManager` 于 `Abstractions/WindowManager/`）、`Framework`（`RegisterMenus` 扩展于 `Framework/Menus/`）、`Resource`（`Language`）、`UIPackage`（`Icons`）——**不引用 shell 宿主 Workstation**。模块对 shell 的集成全靠 Core/Abstractions 的贡献机制（实现 `IMainViewContribution`、`IPanelTabContribution` 等接口；菜单用 `[MenuGroup]`/`[MenuItem]` attribute）加 `IEventAggregator` 事件（`OpenMainViewEvent` 等），因此模块编译期与 shell 实现完全解耦。
...
```
DashBoardModule.RegisterTypes（DashBoardModule.cs:9-21）
  → 容器注册 5 个 I*Contribution 单例 + 4 个视图类型；`RegisterMenus(typeof(DashBoardModule).Assembly)`（第 15 行）扫描程序集内 `[MenuGroup]` 类，注册 `DashBoardMenus` 单例并为每个 `[MenuItem]` 方法注册一个 `IMenuItemContribution` 工厂
  → ShellContributionCollector（Core/Framework）收集（导航/主视图/面板/状态栏按 Placement/Panel 分组、按 Order 排序）；菜单贡献由 `MenuTreeBuilder.Build` 按 Path/Group/GroupOrder/Order 建树，组间插分隔线
...
1. **要加一个新的 shell 贡献（如第二个面板 tab）**：新建实现 `IPanelTabContribution` 的类（参照 `DashBoardTasksPanelTab.cs`），选好 `Id`（模块名前缀 `dashboard.*`）、`Order`、定位枚举与 `ContentViewType`；在 `DashBoardModule.RegisterTypes`（DashBoardModule.cs:11-16 区块）加一行 `RegisterSingleton<IPanelTabContribution, 新类>()`；再建对应视图 `UserControl` 并 `containerRegistry.Register<新视图>()`。菜单项不走接口——在 `[MenuGroup]` 标记的菜单类上加 `[MenuItem]` 方法即可（参照 `DashBoardMenus.cs`），无需在 `RegisterTypes` 加行。标题与图标分别在 Core/Resource 的 `Language` 与 Core/UIPackage 的 `Icons` 中新增。
...
5. **要改模块注册的内容**：全部集中在 `DashBoardModule.RegisterTypes`（DashBoardModule.cs:9-21）。注意不要在 `OnInitialized` 里加开窗逻辑——启动台由启动序列负责（ADR-0004，见 pitfalls.md）。
