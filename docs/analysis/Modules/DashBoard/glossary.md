...
| **ADR-0002** | 架构决策记录：「工具视图统一为 ToolView：Attribute 声明、跨 Bar 拖拽、布局持久化」（docs/adr/0002-toolview-drag-persistence.md）。`INavigationItemContribution`/`IPanelTabContribution` 合并删除，改为 View 类上的 `[ToolView]` attribute + `RegisterToolViews(Assembly)` 扫描注册；工具视图可在三处 Bar 间拖拽，布局持久化 | `DashBoardModule.cs:11` 注释；两个 `[ToolView]` 标注处 |
...
| **ADR-0004** | 架构决策记录编号：确立"启动台 + 逐模块异步加载 + 失败可决策"的启动模型。是 `OnInitialized` 空实现与整个启动台存在方式的理由。**ADR 文档本体不在仓库中**（悬空引用） | `DashBoardModule.cs:23` 注释；README 第 47-51 行 |
