...
| **贡献（Contribution）** | 模块向 shell 提供的声明式条目：四种为实现 Abstractions `I*Contribution` 接口并注册到容器的纯属性类；菜单项为 `[MenuGroup]`/`[MenuItem]` attribute 标记的菜单类，由 Framework `RegisterMenus` 反射注册（ADR-0001）。均由 shell 收集排序/建树渲染。本模块贡献五种：导航项、主视图、面板 tab、菜单项、状态栏项 | `DashBoardModule.cs:11-16`；接口定义在 Core/Abstractions/Contributions/，菜单 attribute 在 Core/Abstractions/Menus/ |
