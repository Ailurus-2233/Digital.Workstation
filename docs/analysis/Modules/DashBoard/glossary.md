...
| **ADR-0002** | 架构决策记录：「工具视图统一为 ToolView：Attribute 声明、跨 Bar 拖拽、布局持久化」（docs/adr/0002-toolview-drag-persistence.md）。`INavigationItemContribution`/`IPanelTabContribution` 合并删除，改为 View 类上的 `[ToolView]` attribute + `RegisterToolViews(Assembly)` 扫描注册；工具视图可在三处 Bar 间拖拽，布局持久化 | `DashBoardModule.cs:13` 注释；两个 `[ToolView]` 标注处 |
| **ADR-0005** | 架构决策记录：「命令体系：Attribute 注册、全局命令列表与命令面板」（docs/adr/0005-command-registration-palette.md）。`CommandAttribute`（标方法、免类级 attribute）+ `ICommandContribution` + Framework `RegisterCommands` 扫描注册；命令面板（Ctrl+P）检索执行；命令与菜单两套独立声明 | `DashBoardCommands.cs`；`DashBoardModule.cs:20` |
| **ADR-0004** | 架构决策记录编号：确立"启动台 + 逐模块异步加载 + 失败可决策"的启动模型。是 `OnInitialized` 空实现与整个启动台存在方式的理由。**ADR 文档本体不在仓库中**（悬空引用） | `DashBoardModule.cs:26` 注释；README 第 47-51 行 |
