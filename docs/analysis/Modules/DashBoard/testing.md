...
5. 贡献类的声明值：`DashBoardTasksPanelTab.Order==15 && Panel==PanelPlacement.Bottom`、`OpenDashBoardMenuItem.Order==10 && Menu==MenuPlacement.File` 等纯属性断言（价值低，防误改排序约定时才有意义）。
...
3. 工作区内：ActivityBar 出现"启动台"项；点开 SideBar 出现"概览/最近项目"两个条目，点击后 MainContent 整体切换；BottomPanel 出现"任务"tab 且排在"输出"与"日志"之间；文件菜单出现"打开启动台"且位于"退出"之前，点击重新打开启动台窗口；状态栏出现启动台条目且位于"就绪"之后。
