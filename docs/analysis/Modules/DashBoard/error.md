...
1. **`IoC.Provider` 未初始化时构造 `DashBoardNavigationView` 失败** — 触发位置：无参构造 `DashBoardNavigationView()` → `IoC.Provider.Resolve<IEventAggregator>()`（Views/DashBoardNavigationView.axaml.cs:18）。触发条件：XAML runtime loader（设计器、预览器）在容器初始化前实例化视图。`IoC.Provider` 的未初始化行为定义在 Core/Common（见 docs/analysis/Core/Common/ 文档）。排查：确认该视图的实例化路径——shell 正常渲染走容器 `Resolve<DashBoardNavigationView>`（调用注入构造，不经无参构造）；只有 XAML loader 直建实例才走无参构造。
2. **`IWindowManager` 相关异常（来自 Core/Framework 实现侧）** — 触发位置：`OpenDashBoardMenuItem` 的 `Command` 执行 → `windowManager.ShowWindow<DashBoardWindow>`（OpenDashBoardMenuItem.cs:18）。可能异常：
...
| 点击"概览/最近项目"主视图不切换 | `OpenMainViewEvent` 负载 Id 与 `IMainViewContribution.Id` 不匹配——负载来自 `DashBoardOverviewMainView.ViewId`/`DashBoardRecentMainView.ViewId` 常量（DashBoardNavigationView.axaml.cs:30、35），改 Id 字符串必须同步改常量引用处 | 比对 `"dashboard.overview"`/`"dashboard.recent"` 在贡献类与订阅方（shell）两侧是否一致 |
| 菜单"打开启动台"点击无反应或报错 | `IWindowManager` 未解析到（构造注入失败会在模块加载期暴露）；或窗口已注册在 `_windowMap` 的实例状态异常 | Core/Framework `FrameworkWindowManager.ShowWindow` 路径；窗口是否已被关闭并从映射移除（关闭后可再次 Show） |
| 贡献项不显示或顺序不对 | `Id` 冲突（跨模块唯一性约束，见 docs/analysis/Core/Abstractions/error.md）；`Order` 值与其他贡献相同导致相对顺序不稳 | `ShellContributionCollector`（Core/Framework）的过滤与排序；全仓搜索重复 Id 字符串 |
...
- **进入模块的异常**：`RegisterTypes` 与贡献类属性求值中的异常 → 冒泡到 Prism 模块初始化 → 启动序列捕获 → `ModuleLoadFailedEvent`（含 `ErrorMessage`）→ 本模块 `DashBoardWindowViewModel.OnModuleFailed` 显示。
