...
| 接口贡献项不显示或顺序不对（菜单项见上行） | `Id` 冲突（跨模块唯一性约束，见 docs/analysis/Core/Abstractions/error.md）；`Order` 值与其他贡献相同导致相对顺序不稳。菜单项无 Id/全局 Order，位次由 `[MenuGroup]`/`[MenuItem]` 的 Path/Group/GroupOrder/Order 经 `MenuTreeBuilder` 建树决定 | `ShellContributionCollector`（Core/Framework）的过滤与排序；全仓搜索重复 Id 字符串；菜单看 `MenuTreeBuilder` 的分组/排序规则 |
...
- **进入模块的异常**：`RegisterTypes` 与贡献类属性求值中的异常 → 冒泡到 Prism 模块初始化 → 启动序列捕获 → `ModuleLoadFailedEvent`（含 `ErrorMessage`）→ 本模块 `DashBoardWindowViewModel.OnModuleFailed` 显示。例外：`RegisterMenus` 对非法 `[MenuItem]` 签名与空段路径只记 `Logger.Warning` 跳过，不抛异常。
