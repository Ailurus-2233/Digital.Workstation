...
1. **应用引导与启动序列**：抽象基类 `FrameworkApplication<TWindow>`（`FrameworkApplication.cs:20`）继承 Prism.DryIoc 的 `PrismApplication`，装载主题、注册框架服务、执行"启动台 → 逐模块异步加载 → 显示主窗口"的三阶段启动序列（ADR-0004）。
...
6. **布局持久化**：`LayoutPersistence`（`Layout/LayoutPersistence.cs:12`，ADR-0002）负责 %AppData%/Digital.Workstation/layout.json 的读/防抖写/删，`ShellLayoutDto` 族（`Layout/ShellLayoutDto.cs:10`）是落盘专用 DTO record（独立于 `ShellLayoutState`）；机制在 Framework、接线在 shell 模块——由 `RegisterFrameworkServices` 注册单例（`FrameworkApplication.cs:159`），真实的捕获/恢复/重置接线在 Modules/Workstation 的 `MainWindowViewModel`。
7. **设置注册与持久化**（ADR-0006）：`SettingRegistration.RegisterSettings`（`Settings/SettingRegistration.cs:19`）与 `RegisterMenus`/`RegisterCommands` 同构——注册期只扫调用方程序集一次，把 `[SettingGroup]`/`[SettingItem]` 声明落成 `SettingGroupContribution`/`SettingItemContribution` 注册进容器（同名分组的合并在收集侧）；`SettingsService`（`Settings/SettingsService.cs:17`）实现 `ISettingsService`——启动一次 `Load()` 把 settings.json 读入内存、读走内存（未修改回退声明默认值）、写=更新内存+防抖落盘+广播 `SettingChangedEvent`，并跟踪「值是否偏离启动时生效值」支撑「重启后生效」判定（决策 7）；`UiLanguage`（`Settings/UiLanguage.cs:12`）与 `GeneralSettings`（`Settings/GeneralSettings.cs:10`）预置「常规/语言」设置项（zh-CN/en-US，需重启）；`ApplicationRestarter`（`ApplicationRestarter.cs:14`）承载「立即重启」——强制落盘后启动新进程并退出当前进程。
...
- **菜单栏按平台呈现**：`FrameworkWindow` 在 macOS 不创建 Ursa 标题栏内的 `Menu`，避免菜单文字与左上角 traffic-light 窗口按钮重叠；shell 完成贡献收集后调用 `RegisterNativeMenu`，把同一棵 `MenuItemViewModel` 树递归映射为 `NativeMenu`/`NativeMenuItem`/`NativeMenuItemSeparator` 并附加到窗口，菜单进入屏幕顶部的 macOS 系统菜单栏。其他平台继续使用 `LeftContent` 中的 `Menu.chrome-menu`，现有样式与行为不变。
...
- **设置管线：扫描一次、读走内存、防抖落盘、事件广播**（ADR-0006 决策 3）：`SettingsService`（`Settings/SettingsService.cs:17`）启动时经 `Load()`（:64）把 settings.json 一次性读入内存镜像 `_values`（缺失/损坏记 Warning 按无修改处理，容错仿 LayoutPersistence），载入内容同时复制为 `_sessionStartValues`——「重启后生效」判定的基准快照（决策 7）；`Get`（:97）纯内存读，未修改时回退**声明的默认值**——默认值不在存储层，来自容器中的 `SettingItemContribution`，经 `FindContribution`（:179）惰性按 Id 缓存、缓存未命中重新枚举容器（允许模块后到注册）；`Set`（:126）= 更新内存 + 500ms `Timer` 防抖落盘 + `TrackPendingRestart`（:158，当前值与启动值经 `JsonElement.DeepEquals` 比较，偏离记入 `_pendingRestartIds`、改回即移出）+ 广播 `SettingChangedEvent`（:143）。`IsPendingRestart`（:146）查询该集合；`FlushPending`（:198）立即落盘（停防抖 Timer + 同步写盘），供「立即重启」在启动新进程前调用（`ApplicationRestarter.Restart`，`ApplicationRestarter.cs:19`）——不强制落盘则新进程可能读到防抖窗口内的旧配置。`Flush`（:208）是 Timer 回调，写盘经 `Save`（:222）全 catch，纪律同 LayoutPersistence。配套启动时序：`ApplyLanguageSetting`（`FrameworkApplication.cs:200`）在 `RegisterFrameworkServices` 内、一切模块 `RegisterTypes` 之前执行——菜单/工具视图标题在模块注册扫描时经 `Language.Get` 一次性解析定死，语言必须先就位；同时设 `DefaultThreadCurrentCulture/UICulture` 与 `Current`（:203-206），前者覆盖阶段 2 `Task.Run` 的模块加载线程；无存值回退声明默认值 zh-CN，与操作系统区域性无关。
...
Avalonia 启动
  ...
  → RegisterTypes()                              [FrameworkApplication.cs:189]
      → RegisterFrameworkServices(): IoC.Initialize(registry, Container)；
        ...
        （必须先于模块 RegisterTypes——菜单/工具视图标题注册期经 Language.Get 解析定死）
...
9. **要改命令面板行为**（过滤规则、MRU、键盘导航、空态）：全部在 `Windows/CommandPalette.cs`（ADR-0005）——交互逻辑自包含在控件内；外观（宽度/位置/颜色）在 `FrameworkWindowTheme.axaml:630-667` 的 `windows|CommandPalette` 样式族。要改数据源排序/去重才去 `ShellContributionCollector.GetCommands`（:42）；要给命令加快捷键改模块侧 `[Command(Gesture = "...")]`，接线（shell 调 `RegisterCommandGestures`）已在 `MainWindow.axaml.cs` 的 `OnOpened`。
