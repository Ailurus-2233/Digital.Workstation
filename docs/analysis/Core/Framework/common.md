...
- **设置管线：扫描一次、读走内存、防抖落盘、事件广播**（ADR-0006 决策 3）：`SettingsService`（`Settings/SettingsService.cs:17`）启动时经 `Load()`（:53）把 settings.json 一次性读入内存镜像 `_values`（缺失/损坏记 Warning 按无修改处理，容错仿 LayoutPersistence）；`Get`（:85）纯内存读，未修改时回退**声明的默认值**——默认值不在存储层，来自容器中的 `SettingItemContribution`，经 `FindContribution`（:135）惰性按 Id 缓存、缓存未命中重新枚举容器（允许模块后到注册）；`Set`（:114）= 更新内存 + 500ms `Timer` 防抖落盘（`Flush` :150 是 Timer 回调必须全 catch，纪律同 LayoutPersistence）+ 广播 `SettingChangedEvent`（:128）。配套启动时序：`ApplyLanguageSetting`（`FrameworkApplication.cs:200`）在 `RegisterFrameworkServices` 内、一切模块 `RegisterTypes` 之前执行——菜单/工具视图标题在模块注册扫描时经 `Language.Get` 一次性解析定死，语言必须先就位；同时设 `DefaultThreadCurrentCulture/UICulture` 与 `Current`（:203-206），前者覆盖阶段 2 `Task.Run` 的模块加载线程；无存值回退声明默认值 zh-CN，与操作系统区域性无关。
