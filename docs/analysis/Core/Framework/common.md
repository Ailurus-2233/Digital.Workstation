...
9. **要改拖拽语义**（迁移/重排/显隐联动/回退规则）：只改 `ShellLayoutState.MoveTab`（`Layout/ShellLayoutState.cs:109`）——语义全在这个纯函数里；控件侧（`ToolViewButton`/`ToolViewBar`）只管手势与落点视觉，不要往里加逻辑。要改落点视觉（占位线样式/插入序号计算）才去 `ToolViewBar` 与其 ControlTheme（`FrameworkWindowTheme.axaml:479`）。
