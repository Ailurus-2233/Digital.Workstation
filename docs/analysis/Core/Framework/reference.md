...
- `NodeAccum`：构造参为稳定 Id 段，Children 按 Ordinal 段索引；Items 保存叶子，DeclaredTitle 保存首个非 null 末端标题，Title 无声明时回退 Id 段。MergeNodeOrder/MergePlacement 合并最小位次，与标题声明独立。

关系要点：`MenuTreeBuilder.Build` 输入收集器扁平列表，输出 `IReadOnlyList<MenuTreeSubmenu>`；它只消费已解析 Title/可空 PathTitle，不查资源。`MenuGroup(path)` 可无标题挂接已有路径，所有者标题后到仍可命名节点；不同稳定路径不会因标题相同而合并。`MenuItemViewModel.FromSubmenu` 把树递归转换为呈现模型，由 FrameworkWindow 按平台映射到原生或标题栏菜单。
