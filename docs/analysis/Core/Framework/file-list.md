...
`public class ToolViewBar : ItemsControl`（第 19 行，ADR-0002）：工具视图 Bar 的投放目标（ActivityBar 顶部段与两个面板 tab 条；底部钉住段不用本控件，自然禁止光标）。StyledProperty：`TargetBarProperty`（:33）、`OrientationProperty`（:36，决定插入序号轴向与占位线方向）、`MoveCommandProperty`（:39）。构造（:44）：`DragDrop.SetAllowDrop(this, true)` + AddHandler 挂 DragOver/Drop/DragLeave。**不得声明 StyleKeyOverride**——Avalonia 类型选择器匹配 StyleKey 而非运行时类型，override 会让 `layout|ToolViewBar` 选择器与 ControlTheme 查找失效；其 ControlTheme 在 FrameworkWindowTheme.axaml（透明背景使命中测试覆盖整条带、模板含 `PART_InsertionLine`）。`OnApplyTemplate`（:75）按 Orientation 配置占位线横/竖（横向 Bar 竖线 2px、纵向 Bar 横线 2px，`InsertionLineThickness` :26）。`ComputeInsertionIndex`（:139，指针在条目前半→插其前，否则末尾）；`ShowInsertion`（:163，经 `TranslatePoint` 把落点缝隙坐标换算为占位线 Margin）；`OnDragLeave`（:131）有冒泡守卫——指针真正离开本 Bar 才清除。
...
无 x:Class 的 Styles 根（616 行）。`Styles.Resources`：`NavigationItemTemplate`（:8，按钮为 ToolViewButton，绑 CanDrag/DragTabId）、六个共享部件 DataTemplate（`ShellActivityBar` :25 / `ShellSideBar` :46 / `ShellMainContent` :62 / `ShellAuxiliaryPanel` :74 / `ShellBottomPanel` :129 / `ShellStatusBar` :185——三处可投放 Bar 用 layout:ToolViewBar 承载并绑 MoveCommand/TargetBar/Orientation，ActivityBar 底部钉住段保持普通 ItemsControl）、四份布局 DataTemplate（`WindowLayoutLeft` :214 / `WindowLayoutRight` :280 / `WindowLayoutCenter` :346 / `WindowLayoutJustify` :411，差异为 BottomPanel 及分隔条的 `Grid.Column`/`ColumnSpan` 与侧栏的 `Grid.RowSpan`，ActivityBar 恒 `RowSpan=2` 通高）、`ToolViewBar` 的 ControlTheme（:479，透明背景使整条带参与命中测试，模板含 `PART_InsertionLine` 拖拽占位线）。样式（:500 起）自 Modules/Workstation/MainWindow.axaml 迁入；`layout|ToolViewBar.drag-over`（:541）为拖拽悬停整 Bar 高亮；Auxiliary/Bottom 面板卡片 `IsVisible` 绑 `AuxiliaryPanelRevealed`/`BottomPanelRevealed`（拖拽会话期间临时显露隐藏面板，ADR-0002）。末尾 4 个标题栏菜单样式（:598-616）：顶层 MenuItem 紧凑行高、Popup#PART_Popup VerticalOffset=-8 贴合标题栏下缘、`Menu.chrome-menu MenuItem` 的 ItemsSource/Command/AutomationProperties.Name 样式绑定、PathIcon 前景色。
