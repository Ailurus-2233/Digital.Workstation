...
- **以为 `Placements` 覆盖全部工具视图**：钉住项（`AllowMove=false`）恒落 ActivityBar 底部段，**不入** placements 表（`ShellLayoutDto.cs:25-27` 注释；写入端 `MainWindowViewModel.CaptureLayout` 只采集 TopNavigationItems/AuxiliaryTabs/BottomTabs 三个可移动集合，`Modules/Workstation/MainWindowViewModel.cs:536` 起）；恢复端的分派也不从表里找钉住项（`LoadToolViews` :225-226 直接按 attribute 过滤）。改持久化格式时不要把钉住项写进表。
