...
3. **共享图标几何**：`Icons`（`Icons.cs:7`）静态类集中存放 14 个 `public const string` 的 StreamGeometry path 字符串（`Settings`、`DashBoard`、`ChevronDown`、`AlignLeft`……），各模块的贡献类（导航项、面板 tab、菜单项、状态栏项）通过 `IconPath => Icons.Xxx` 引用，不在各自类中硬编码 path。
