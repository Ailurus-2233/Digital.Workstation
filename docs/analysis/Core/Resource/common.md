...
4. **强类型门面：`nameof` 保证键与属性同名**。30 个静态属性（`Language.cs:25-168`）每个都是 `=> Get(nameof(XxxTitle))`。`nameof` 让键名与属性名永远一致：重命名属性时编译器会跟着改键名表达式，但 **`.resx` 里的 `data name` 必须手动同步改**（见 pitfalls.md）。
...
4. **新增一种语言（如 ja-JP）**：新增 `Language.ja-JP.resx`，逐条翻译 30 个键；`Language.cs` 无需改动，`ResourceManager` 按 `CurrentUICulture` 自动选取。新语言的键可以只翻译一部分，缺失的键回退到中文。
