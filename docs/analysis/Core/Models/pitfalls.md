...
- **加枚举成员后编译全绿但行为错**：`MainWindowViewModel.TogglePanel`（第 249-256 行）、`TogglePanelContribution` 的 Title/Icon/Order（第 30-48 行）、`DashBoardWindowViewModel.OnProgress`（第 41-47 行）全部用 `_` 兜底分支。给 `TogglePanelTarget` 加新成员后，菜单项 Id/标题/图标/排序和实际翻转的面板会静默落到兜底值（如都显示底部面板图标、都翻转 BottomPanel）。C# switch 表达式对枚举无穷尽性强制。
