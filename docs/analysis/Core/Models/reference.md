...
事件类的基类 `Prism.Events.PubSubEvent<T>`（`ResetLayoutEvent` 用非泛型 `PubSubEvent`）来自 Prism 包。`Core/Models/obj/Debug/Models.GlobalUsings.g.cs` 第 2-11 行含 Prism 包 build props 注入的 global using（`Prism`、`Prism.Events`、`Prism.Ioc` 等），其中**第 6 行 `global using Prism.Events;`** 就是 13 个源码文件不写任何 `using` 也能解析 `PubSubEvent<T>` 的原因。
...
- **[ADR-0004](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0004-startup-sequence.md)**：`StartupProgressEvent`、`ModuleLoadFailedEvent`、`StartupPhase` 的注释及 README 第 47-51 行均引用之；决策记录见 `docs/adr/0004-startup-sequence.md`，说明抑制 Prism 同步 `InitializeModules`、改由启动序列逐模块异步加载，以及进度与失败决策事件。
...
| 消费方 | 引用方式 | 消费点 |
|---|---|---|
| `Core/Framework`（Framework.csproj 第 12 行直接引用） | 直接 | `FrameworkApplication.cs`：发布 `StartupProgressEvent`（第 73/78/89/108 行）、发布 `ModuleLoadFailedEvent`（第 97-98 行）、在 `WaitForFailureActionAsync` 订阅 `StartupFailureActionEvent`（第 121-129 行）——本模块全部启动事件的唯一发布中枢；`Settings/SettingsService.cs`：`Set` 第 128 行发布 `SettingChangedEvent`（[ADR-0006](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0006-attribute-settings-registration.md) 决策 3） |
...
本模块无 internal 类型，全部 13 个 public 类型的关系图：
...
Prism.Events.PubSubEvent<T>（Prism 包）
  ...
  ├─ SettingChangedEvent         ──负载──▶ SettingChanged (record: SettingId/NewValue，[ADR-0006](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0006-attribute-settings-registration.md) 决策 3)
  └─ ResetLayoutEvent            ──无负载（非泛型 PubSubEvent，[ADR-0002](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0002-toolview-drag-persistence.md)）
```
...
| `ResetLayoutEvent` | class : `PubSubEvent`（无成员体、无负载） | Events/ResetLayoutEvent.cs | 重置布局请求（[ADR-0002](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0002-toolview-drag-persistence.md)） |
| `SettingChangedEvent` | class : `PubSubEvent<SettingChanged>`（无成员体） | Events/SettingChangedEvent.cs | 设置项变更事件（[ADR-0006](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0006-attribute-settings-registration.md) 决策 3） |
