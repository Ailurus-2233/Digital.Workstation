...
| **Contribution（贡献）** | 本模块的核心模式：模块向 shell 贡献 UI 条目，shell 收集后渲染。接口类贡献三种：主视图、菜单项、状态栏项；导航项与面板 tab 已合并为**工具视图**（attribute 声明 + 扫描生成元数据，[ADR-0002](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0002-toolview-drag-persistence.md)）。菜单项通常不手写实现，由 `MenuGroupAttribute`/`MenuItemAttribute` 标注的菜单类经 `RegisterMenus` 扫描生成（[ADR-0001](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0001-attribute-menu-registration.md)） | `I*Contribution` 接口与 `ToolViewContribution`（Contributions/ 与 Menus/ 目录） |
...
| **ViewType** | 激活贡献项时要显示的视图的 `System.Type`，经容器解析以支持 DI。`ToolViewContribution.ViewType` 与 `IMainViewContribution.ViewType` 命名已统一（原 `ContentViewType` 随导航项/面板 tab 接口删除而不复存在，[ADR-0002](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0002-toolview-drag-persistence.md)） | Contributions/ToolViewContribution.cs 第 44 行、Contributions/IMainViewContribution.cs 第 19 行 |
