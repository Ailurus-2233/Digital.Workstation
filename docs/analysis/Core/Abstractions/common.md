...
1. **Shell 贡献契约**（`Shell/` 目录）：模块向 shell 界面区域（ActivityBar、SideBar、MainContent、AuxiliaryPanel、BottomPanel、菜单栏、状态栏）注入条目的接口，配合 `ShellRegions` 常量类声明 Prism Region 名称。共 5 个贡献接口：`INavigationItemContribution`、`IMainViewContribution`、`IPanelTabContribution`、`IMenuItemContribution`、`IStatusBarItemContribution`。
...
- **纯契约、零实现**：本项目只有接口、枚举、常量和一个静态扩展类。实现落在 shell 宿主与各模块中。这是典型的「抽象在 Core、实现在别处」的分层方式，使模块只依赖本程序集即可参与 shell 的 UI 组合，不依赖 shell 实现。
- **贡献点模式（contribution pattern）**：五个 `I*Contribution` 接口结构高度同构——`Id`（稳定标识）+ `Title` + `IconPath` + `Order` + 定位字段（`Placement`/`Menu`/`Panel`）+ 行为字段（`ContentViewType`/`ViewType`/`Command`）。模块在 `Prism.Ioc.IContainerRegistry` 中以接口注册实现，shell 收集全部实现后按 `Order`（小者靠前）渲染。选择该模式而非直接 new 视图，是为了让模块**声明式**地扩充 UI，shell 无需知道有哪些模块。
...
- Shell 贡献链：模块在 DI 容器注册 `I*Contribution` 实现 → shell 启动时收集全部实现 → 按定位字段（`Placement`/`Menu`/`Panel`）分组、按 `Order` 排序 → 渲染到 `ShellRegions` 声明的 Region。用户交互（点击导航项/菜单项、打开主视图事件 `OpenMainViewEvent` 负载为 `IMainViewContribution.Id`）触发 shell 解析 `ContentViewType`/`ViewType` 或执行 `IMenuItemContribution.Command`。
...
1. **新增一种 shell 贡献类型**（如「工具栏按钮贡献」）：在 `Shell/` 下新建 `IToolBarItemContribution.cs`，仿照 `IMenuItemContribution.cs` 的字段结构（`Id`/`Title`/`IconPath`/`Order` + 定位枚举 + 行为字段），放入命名空间 `DigitalWorkstation.Core.Abstractions.Shell`；若需要新 Region，在 `Shell/ShellRegions.cs` 加 `public const string X = nameof(X);`。
2. **给现有贡献加元数据**（如菜单项加快捷键提示）：改对应接口文件（如 `Shell/IMenuItemContribution.cs`）增加属性。**注意这会同时破坏所有实现方与 shell 收集方**，属于跨程序集破坏性变更；本模块是解决方案最底层契约，任何签名改动都会级联。
...
4. **改 Region 名称**：只改 `Shell/ShellRegions.cs` 中的 `nameof` 目标；由于值来自 `nameof`，重命名常量本身时引用方编译安全，但**序列化/配置中已存的 Region 名字符串**会失配。
5. **改图标机制**：`IconPath` 出现在全部 5 个贡献接口中（`Shell/I*Contribution.cs`），要换图标来源需一次性改 5 个文件。
