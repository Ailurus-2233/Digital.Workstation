...
2026-09-15 修订：命令标题显式声明资源类型，命令面板自身文案归 Framework；命令标识、执行与交互模型不变。
...
7. `[Command(typeof(OwnerResources), nameof(OwnerResources.Title))]` 显式指定标题资源。标题在收集构造贡献时经 `ResourceText.Get(attribute.ResourceType, attribute.Title)` 一次解析；列表项显示可选图标（`CommandAttribute.Icon` → 契约 `IconPath`，为 null 渲染空 `PathIcon` 占位槽位）+ 标题 + 右侧快捷键文本。输入框水印与空态文案使用 `FrameworkResources`；无 `>`/`#` 前缀模式语法；Esc、失焦、执行命令后关闭面板。
