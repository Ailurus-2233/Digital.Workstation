...
1. 设置项经 attribute 声明，拆两个 attribute 仿菜单模型（[ADR-0001](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0001-attribute-menu-registration.md)）：`SettingGroupAttribute` 标类（声明组名与组 Order，多处声明冲突取最小），`SettingItemAttribute` 标静态属性（携带所属分组、名称、默认值、Order、RequiresRestart）；设置项 Id 默认「声明类全名.属性名」（仿命令 Id 规则）。模块 `RegisterTypes` 显式调用 `RegisterSettings(Assembly)` 扫描本模块程序集，与 `RegisterMenus`/`RegisterCommands` 同构，不做全局程序集扫描。
2. attribute 中的名称与分组字符串是 `Language` 资源键，运行时解析，缺键回退键名本身（同 [ADR-0001](https://github.com/Ailurus-2233/Digital.Workstation/blob/main/docs/adr/0001-attribute-menu-registration.md) 决策 5）。
