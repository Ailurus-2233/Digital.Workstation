...
/// </summary>
[MenuGroup("shell.view", Group = "Alignment", GroupOrder = 200)]
public class ViewAlignmentMenus(IEventAggregator eventAggregator)
{
    [MenuItem(typeof(WorkstationResources), nameof(WorkstationResources.PanelAlignLeftTitle), Order = 100, Icon = Icons.AlignLeft)]
    public void AlignLeft()
    {
        eventAggregator.GetEvent<SetPanelAlignmentEvent>().Publish(PanelAlignment.Left);
    }

    [MenuItem(typeof(WorkstationResources), nameof(WorkstationResources.PanelAlignRightTitle), Order = 200, Icon = Icons.AlignRight)]
    public void AlignRight()
    {
        eventAggregator.GetEvent<SetPanelAlignmentEvent>().Publish(PanelAlignment.Right);
    }

    [MenuItem(typeof(WorkstationResources), nameof(WorkstationResources.PanelAlignCenterTitle), Order = 300, Icon = Icons.AlignCenter)]
    public void AlignCenter()
    {
        eventAggregator.GetEvent<SetPanelAlignmentEvent>().Publish(PanelAlignment.Center);
    }

    [MenuItem(typeof(WorkstationResources), nameof(WorkstationResources.PanelAlignJustifyTitle), Order = 400, Icon = Icons.AlignJustify)]
